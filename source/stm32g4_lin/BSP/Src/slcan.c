...
static uint8_t transmitStd(uint8_t *line) {
	...
	if (line[0] < 'Z')
		offset = 5;
	// id
	if (!parseHex(&line[2 + offset], 2, &temp))
		return 0;
	slot.pid = open_lin_data_layer_parity((open_lin_pid_t) temp); // add parity
	printf("slot.pid: 0x%02X 0x%02X\n", slot.pid, open_lin_data_layer_parity((open_lin_pid_t) slot.pid));
	// len
	if (!parseHex(&line[4 + offset], 1, &temp))
		return 0;
	slot.data_length = temp;

	if (slot.data_length > 8)
		return 0;
	if (lin_data) {
		uint8_t i;
		for (i = 0; i < slot.data_length; i++) {
			if (!parseHex(&line[5 + offset + i * 2], 2, &temp))
				return 0;
	...
	if (offset == 0) {
		l_bool ret = open_lin_master_data_tx_header(&slot);
		(void)ret;
		slcanPrintf("send master tx header PID:0x%02X, %s\n", slot.pid, ret?"OK":"FAILED");
	}
	if (lin_data) {
		l_bool ret = open_lin_master_data_tx_data(&slot);
		(void)ret;
		slcanPrintf("send master tx data PID:0x%02X, Type:0x%02X, len:%d, %s\n", slot.pid, slot.frame_type, slot.data_length, ret?"OK":"FAILED");
		for(uint8_t i=0;i<slot.data_length;i++)
		{
			slcanPrintf("0x%02X ", (uint8_t)slot.data_ptr[i]);
		}
		slcanPrintf("\n");
	}
...
void USART1_UART_Init(void) {
	...
	if (HAL_LIN_Init(&huart1, UART_LINBREAKDETECTLENGTH_11B) != HAL_OK) {
		slcanPrintf("failed HAL_LIN_Init\n");
	}
	if (HAL_UARTEx_SetTxFifoThreshold(&huart1, UART_TXFIFO_THRESHOLD_1_8)
			!= HAL_OK) {
		slcanPrintf("failed HAL_UARTEx_SetTxFifoThreshold\n");
	}
	if (HAL_UARTEx_SetRxFifoThreshold(&huart1, UART_RXFIFO_THRESHOLD_1_8)
			!= HAL_OK) {
		slcanPrintf("failed HAL_UARTEx_SetRxFifoThreshold\n");
	}
	if (HAL_UARTEx_DisableFifoMode(&huart1) != HAL_OK) {
		slcanPrintf("failed HAL_UARTEx_DisableFifoMode\n");
	}
}

static void enableLinBus(bool on) {
...
void slCanCheckCommand() {
	...
	const char *testR = "r02b8";
	const char *testT = "t02881122334455667788";
	if(line[0] == '1')
	{
		for(uint8_t i=0;i<strlen(testR);i++) {
			line[i] = testR[i];
		}
	}
	else if(line[0] == '2')
	{
		for(uint8_t i=0;i<strlen(testT);i++) {
			line[i] = testT[i];
		}
	}
	switch (line[0]) {
	case '0':
		...
		slcanSetOutputChar(slcan_state+0x30);
		break;
	case 'a': {
		if (terminator == SLCAN_CR)
			...
			terminator = SLCAN_CR;
		result = terminator;
		slcanPrintf("set the terminator character %c\n", terminator);
		break;
	}
	case 'S':
	case 'G':
	...
	case 's':
		...
		open_lin_hw_reset();
		result = terminator;
		slcanPrintf("reset lin bus. baudrate: %ldbps\n", lin_baund_rate);
		break;
...
//                slcanSetOutputAsHex(VERSION_HARDWARE_MAJOR);
	...
	case 'O':
		if (slcan_state == SLCAN_STATE_CONFIG) {
			...
			lin_slcan_reset();
			enableLinBus(true);
			slcanPrintf("open the master mode\n");
		}
		else {
			slcanPrintf("cant open the master mode. please change the state to Closed\n");
		}
		break;
	case 'L': // slave mode
		if (slcan_state == SLCAN_STATE_CONFIG) {
			...
			lin_slcan_reset();
			enableLinBus(true);
			slcanPrintf("open the slave mode\n");
		}
		else {
			slcanPrintf("cant open the slave mode. please change the state to Closed\n");
		}
		break;
	case 'l':  // monitor
		if (slcan_state == SLCAN_STATE_CONFIG) {
			...
			lin_slcan_reset();
			enableLinBus(true);
			slcanPrintf("open the monitor mode\n");
		}
		else {
			slcanPrintf("cant open the monitor mode. please change the state to Closed\n");
		}
		break;
	...
	case 't': // Transmit full frame
		...
		if (line[1] == '3') {
			...
			result = terminator;
			slcanPrintf("set the crc %s\n",classicChecksum==0?"classic":"extended");
			break;
		}
		...
		if (line[2] == '4') {
			...
			slcanSetOutputChar('Z');
			result = terminator;
			slcanPrintf("set the baudrate %ldbps\n",temp);
			break;
		}
		...
		case LIN_SLAVE:
			if (addLinMasterRow(line) == 1) {
				...
				slcanPrintf("failed to the send message\n");
			}
			result = terminator;
			break;
		case LIN_MONITOR:
